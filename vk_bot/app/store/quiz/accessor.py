...
class QuizAccessor(BaseAccessor):
    ...
    async def get_question_by_title(self, title: str) -> Optional[Question]:
        ...
        return Question(
            ...
            media_id=question.points

        )
    
    async def get_question_by_id(self, theme_id: int, points: int) -> Optional[Question]:

        query = (select(QuestionModel)
                .where(and_(QuestionModel.theme_id == theme_id,
                            QuestionModel.points == points)))
    ...
    async def list_questions_lap(self, theme_raund: Optional[list[int]] = None) -> list[Question]:

        query = select(ThemeModel)

        if theme_raund:
            query = query.where(ThemeModel.id.in_(theme_raund))

        query = query.options(joinedload(ThemeModel.question))
        ...
        return [
            QuestionAndTheme(id=question.id,
                     title=question.title,
                     points=[q.points for q in question.question])
                for question in questions.unique()]
    

    # async def get_question_by_points(self, title: str) -> Optional[Question]:
    ...
    # async def list_questions_lap(self, theme_id: Optional[list[int]] = None) -> list[Question]:


    #     query = select(ThemeModel, QuestionModel).where(ThemeModel.id.in_(theme_id))

    #     # if theme_id:
    #     #     query = query.where(ThemeModel.id.in_(theme_id))
        
    #     # query = query.options(selectinload(ThemeModel.question))
